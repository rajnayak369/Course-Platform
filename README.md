...
- When a user initiates to enrol in a course. A graph based topological sort algorithm is executed. If any circular dependency is detected a error is raised and the course enrolment is aborted. If not found an optimised  path is generated by the algorithm. Further the path is personalised using user details like past course engagements, interests and performance and along with that a recommended parallel enrol courses list is shown.
### Concepts used
